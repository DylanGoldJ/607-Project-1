...
##Codebase Dplyr set up, as well as stringi for parsing strings better
...
player_name <-player_groupings1[,3] %>%
  str_trim() #Because we look for { } or {-} inbetween names we are left with the empty space at the end of the name. str_trim will remove this whitespace
...
We can do this with str_match_all()
I show first player as example.
```{r}
...
Now we make a list of these values
...
```
We tested out some of the rows and everything seems to be working.
Now lets create a data frame with the output information. \
```{r}
