...
#*ADDED player_opponents_status to include match results, for future assignment.*

player_opponents <- vector("list", length(opponent_groupings))
player_opponents_status <- vector("list", length(opponent_groupings))
for(i in 1 : length(opponent_groupings)){
  player_opponents[[i]] <- opponent_groupings[[i]][,3]
  player_opponents_status[[i]] <- paste(opponent_groupings[[i]][,2], opponent_groupings[[i]][,3], sep = "-")
}
...
player_data <- player_data %>% mutate(
  ...
  opponents_results = player_opponents_status # Added for future assignment
)

head(player_data,12) #Data frame is made properly
```
...
```{r}
player_data_output <- player_data %>%
  mutate(opponents = paste(opponents_results, collapse=' ')) %>%
  select(c("number", "name", "state", "points", "rating", "opponents", "avg_opponent_rating"))

player_data_output
write.csv(player_data_output, "player_data.csv", row.names = FALSE) #print, get rid of the index column/row name
```
